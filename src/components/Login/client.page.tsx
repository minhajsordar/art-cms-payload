...
//     return
    if (actionRes.status === 200 && json.user) {
      ...
      // } else {
        if (tenantDomain) {
          router.push(`/tenant-domains/${tenantDomain}`)
        } else {
    ...
    <div className={`${baseClass}`}>
      <form onSubmit={handleSubmit}>
        <div>
          <label>
            Username
            <input className='border border-grey-600' name="username" ref={usernameRef} type="text" />
          </label>
        </div>
        <div>
          <label>
            Password
            <input className='border border-grey-600' name="password" ref={passwordRef} type="password" />
          </label>
        </div>
